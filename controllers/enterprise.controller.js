...
module.exports.getVisitesByEntreprise = async (ETOB_IDENT) => {
  ...
  let visites =  await execSql(visiteQuery).then(response => response.recordsets[0]);
  for (let visite of visites){
    let controleQuery = `select * from CONTROLE 
    where VISITE_IDENT = ${visite.VISITE_IDENT}`
    visite.controles =  await execSql(controleQuery).then(response => response.recordsets[0]);;
  }
  return visites
}
